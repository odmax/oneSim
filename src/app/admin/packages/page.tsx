...
export default async function AdminPackagesPage({
  ...
  // Filter by tab
  let displayPackages = packagesWithReadiness
  if (tab === 'live') {
    displayPackages = packagesWithReadiness.filter(p => p._readiness.ready)
  } else if (tab === 'draft') {
    displayPackages = packagesWithReadiness.filter(p => !p.isActive || p.hiddenFromCatalog || p.archivedAt ||
    ...
    <div className="p-6">
      ...
      <div className="mb-6">
        <nav className="inline-flex rounded-xl bg-gray-100 p-1">
          {TABS.map((t) => {
            const href = t.id === 'live' ? '/admin/packages' : `/admin/packages?tab=${t.id}`
            const isActive = tab === t.id
            const counts: Record<string, number> = { live: livePackages.length, draft: draftPackages.length, 'needs-pricing': needsPricingPackages.length }
            return (
              <Link
                key={t.id}
                href={href}
              ...
              >
                {t.label} <span className={`ml-1 text-xs ${isActive ? 'text-gray-400' : 'text-gray-400'}`}>({counts[t.id] || 0})</span>
              </Link>
            )
          })}
        </nav>
