...
vi.mock('@/lib/services/esims/sync-esim-status', () => ({
  syncESIMStatus: vi.fn(),
}))
...
const mockSyncESIMStatus = vi.mocked(syncESIMStatus)
...
describe('refreshEsimStatus (canonical delegation)', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.mocked(mockSyncESIMStatus).mockResolvedValue({ success: true, status: 'ACTIVE', newStatus: 'ACTIVE', activated: true })
  })

  it('delegates to canonical syncESIMStatus — the SINGLE status refresh service', async () => {
    const result = await refreshEsimStatus('esim-1')
    expect(mockSyncESIMStatus).toHaveBeenCalledWith('esim-1')
  ...
  it('never sends a local OneSIM esim.id — missing safe identifier is a clean skip', async () => {
    vi.mocked(mockSyncESIMStatus).mockResolvedValue({ success: true, skipped: true, skipReason: 'IDENTIFIER_MISSING' })
    const result = await refreshEsimStatus('esim-1')
    expect(result.success).toBe(false)
    expect(result.error).toBe('No provider identifier available for status lookup')
  })

  it('maps a capability-not-supported skip to a friendly error', async () => {
    vi.mocked(mockSyncESIMStatus).mockResolvedValue({ success: true, skipped: true, skipReason: 'STATUS_CAPABILITY_NOT_SUPPORTED' })
    const result = await refreshEsimStatus('esim-1')
    expect(result.success).toBe(false)
    expect(result.error).toBe('Status not supported by provider')
  })

  it('propagates a real provider failure', async () => {
    vi.mocked(mockSyncESIMStatus).mockResolvedValue({ success: false, error: 'boom' })
    const result = await refreshEsimStatus('esim-1')
    expect(result.success).toBe(false)
