  ...
  // ── Airhub Outreach ─────────────────────────────────────────────────────────
  {
    ...
    provider: {
      ...
      config: { ...STAGING_NOTE, providerMode: 'TEMPLATE', templateDriven: true, numericFields: ['partnerCode', 'flag'], arrayFields: ['multiplecountrycode'], responseListKey: 'getInformation', configurationFields: [
        ...
        { key: 'multiplecountrycode', label: 'Countries', type: 'text', required: false, group: 'config', placeholder: 'US,GB,ZA', helperText: 'Comma-separated country codes' },
        { key: 'flag', label: 'Flag', type: 'number', required: false, group: 'config', placeholder: '5' },
        { key: 'partnerCodeOverride', label: 'Partner Code Override', type: 'number', required: false, group: 'config', placeholder: 'Optional', helperText: 'Partner Code is normally read from AirHub login response. Only set if AirHub instructs you to.' },
      ] },
