...
const ORDER_TRANSITIONS: Record<string, string[]> = {
  ...
  PAYMENT_RESERVED: ['PENDING_PROVIDER', 'FAILED', 'CANCELLED', 'PROVIDER_RECONCILIATION'],
  // FULFILLED / PARTIALLY_FULFILLED are legal from PENDING_PROVIDER: async dispatch
  // claims PAYMENT_RESERVED → PENDING_PROVIDER once, then finalizes directly on
  // provider success without intermediate RESERVED/FULFILLING states.
  PENDING_PROVIDER: ['PROVIDER_ACCEPTED', 'RESERVED', 'FAILED', 'PROVIDER_RECONCILIATION', 'FULFILLED', 'PARTIALLY_FULFILLED'],
