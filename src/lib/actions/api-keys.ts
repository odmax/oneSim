...
export async function createApiKey(name: string) {
  try {
    ...
    await prisma.businessApiKey.create({
      data: { businessId: session.user.businessId!, name, keyHash: hash, keyPrefix: prefix },
    })

    await prisma.auditLog.create({
      data: { userId: session.user.id, action: 'API_KEY_CREATED', entity: 'BusinessApiKey', entityId: prefix, details: `API key "${name}" created (prefix: ${prefix})` },
    })
