...
export async function createPackage(formData: FormData) {
  ...
  try {
    ...
    await prisma.eSIMPackage.create({
      data: { name, description, dataGB, validityDays, priceUSD, localPrice, currency },
    })

    await prisma.auditLog.create({
      data: { action: 'CREATE', entity: 'ESIMPackage', details: `Created package: ${name}` },
    })

    revalidatePath('/admin/packages')
  } catch (error: any) {
    handleServerActionError(error, '/admin/packages', 'create_failed')
  }
}
...
export async function updatePackage(formData: FormData) {
  ...
  try {
    ...
    await prisma.eSIMPackage.update({ where: { id }, data: { name, description, dataGB, validityDays, priceUSD, localPrice, isActive } })

    await prisma.auditLog.create({
      data: { action: 'UPDATE', entity: 'ESIMPackage', entityId: id, details: `Updated package: ${name}` },
    })

    revalidatePath('/admin/packages')
  } catch (error: any) {
    handleServerActionError(error, '/admin/packages', 'update_failed')
  }
}
